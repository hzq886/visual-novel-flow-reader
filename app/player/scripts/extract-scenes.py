...
opcode → イベント（content を担う op のみ。flow/flag/timing 命令は beat 非関与で無視）:
  ...
  0x15 <idx>                    se        効果音コード（ワンショット・VOL_SE）
  0x16 <idx> 00                 lpse      ループ se（VOL_LPSE 単一ループチャンネル・sticky・HU-76）
  ...
  0x04                          page      改ページ（テキスト窓クリア・クリック待ち。地の文のページ境界。HU-78）
...
# content を担う op（イベント化する）。0x04（改ページ）はページ境界イベント page（HU-78）。
HANDLED_OPS = {0x01, 0x04, 0x0D, 0x14, 0x15, 0x16, 0x10, 0x12, 0x2C, 0x3B, 0x3C, 0x6A, 0x6C}
...
IGNORED_OPS = {
    0x00, 0x02, 0x03, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x13, 0x17, 0x19, 0x1A,
...
def scan_locale(locale, coverage):
    ...
    for name, size, eo in parse_container(data):
        ...
        for pos, lineno, length, args in records:
            ...
            if op == 0x04:
                # 改ページ（テキスト窓クリア・クリック待ち）。地の文のページ境界（HU-78）。
                events.append(["page"])
            elif op == 0x01 and length == 3:
            ...
            elif op == 0x15 and length == 3:
                events.append(["se", s(struct.unpack_from("<H", args, 1)[0])])
            elif op == 0x16 and length == 4:
                # ループ se（VOL_LPSE チャンネル。0x15 ワンショットとは別。末尾 1 バイトは常に 0x00）。
                # 単一ループチャンネルで sticky 持続＝BGV 同型（停止マーカーは原データに無い・HU-76）。
                events.append(["lpse", s(struct.unpack_from("<H", args, 1)[0])])
