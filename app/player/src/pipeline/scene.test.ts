...
describe('parseScene — [id] ITEM_* をアイテムCG窓（独立オーバーレイ）として反映（HU-70）', () => {
  // items.json 相当（extract-items.py が bytecode 0x3b/0x3c から抽出する仕様表）。
  const ITEMS = {
    '001_PRO001B': {
      item: 'ITEM_03_01',
      x: 700,
      y: 120,
      texts: { jp: 2 },
      nextText: { jp: '窓が閉じた後の地の文。' },
    },
  }

  it('窓表示中は item フィールドに載り、下層の bg/立ち絵はそのまま（texts 消費で閉じる）', () => {
    const text = [
      '[note] #背景・部屋',
      '[note] #綾菜・通常',
      '[text] 直前の地の文。',
      ...
      '[note] #綾菜・驚き', // 原仕様: 窓表示中の立ち絵差分変更（窓は閉じない）
      '[text] 窓表示中の地の文。',
      '[text] 窓が閉じた後の地の文。',
    ].join('\n')
    const s = parseScene(text, { code: '001_PRO001B', locale: 'jp', items: ITEMS })
    expect(
      s.beats.map((b) => ({
        bg: b.bg?.label ?? null,
        sprite: b.sprite?.label ?? null,
      ...
      { bg: '#背景・部屋', sprite: '#綾菜・通常', item: null },
      { bg: '#背景・部屋', sprite: '#綾菜・通常', item: 'ITEM_03_01' }, // 下層は保持
      { bg: '#背景・部屋', sprite: '#綾菜・驚き', item: 'ITEM_03_01' }, // 立ち絵変更で閉じない
      { bg: '#背景・部屋', sprite: '#綾菜・驚き', item: null }, // texts=2 消費で閉じる
    ])
    expect(s.beats[1].item).toEqual({ code: 'ITEM_03_01', file: null, x: 700, y: 120 })
  })

  it('閉じ位置の本文が nextText と一致しなければ throw（txt 再生成とのズレ検知）', () => {
    const text = ['[id] ITEM_03_01', '[text] Ａ。', '[text] Ｂ。', '[text] ズレた本文。'].join('\n')
    expect(() => parseScene(text, { code: '001_PRO001B', locale: 'jp', items: ITEMS })).toThrow(
      /閉じ位置が items\.json と不一致/,
    )
  })

  it('items.json にエントリが無い ITEM マーカーは throw（再生成漏れの検知）', () => {
    expect(() =>
      parseScene('[id] ITEM_11_02\n[text] 壁の器具。', { code: '005_MAKO013C', locale: 'jp' }),
    ).toThrow(/items\.json に無い/)
  })

  it('#背景 変更時は防御的に窓を閉じる（原データでは区間内に出現しない）', () => {
    const text = ['[id] ITEM_03_01', '[text] Ａ。', '[note] #背景・別室', '[text] Ｂ。'].join('\n')
    const s = parseScene(text, { code: '001_PRO001B', locale: 'jp', items: ITEMS })
    expect(s.beats.map((b) => b.item?.code ?? null)).toEqual(['ITEM_03_01', null])
  })
})
...
describe('parseScene — 他シーン流用ボイスIDの取り込み（HU-67）', () => {
  ...
  it('BGV_* / se コード / ITEM_* / BG_BLACK はボイスIDとして誤認しない（既存分岐を維持）', () => {
    ...
    const items = {
      '010_MAIN001A': { item: 'ITEM_01_02', x: 440, y: 120, texts: { jp: 1 }, nextText: {} },
    }
    const s = parseScene(text, { code: '010_MAIN001A', locale: 'jp', items })
    expect(s.beats).toHaveLength(1)
    expect(s.beats[0].kind).toBe('narration')
    // voice ではなく bgv / se / アイテム窓 / 背景として取り込まれている
    expect(s.beats[0].bgv?.id).toBe('BGV_AYAN_H001A')
    expect(s.beats[0].se?.map((x) => x.code)).toEqual(['8351A'])
    expect(s.beats[0].item).toBeUndefined() // 後続 BG_BLACK（被せ）が窓を防御的に閉じる
    expect(s.beats[0].bg?.label).toBe('#背景・黒一色')
  })
})
